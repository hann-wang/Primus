...
if [[ "${PRIMUS_SKIP_PIP:-0}" == "1" ]]; then
  echo "[INFO] PRIMUS_SKIP_PIP=1: skipping Diffusion dependency installation"
  exit 0
fi
...
REQ_FILE="${SCRIPT_DIR}/requirements-diffusion.txt"
if [[ -f "${REQ_FILE}" ]] && grep -qE '^[[:space:]]*[^#[:space:]]' "${REQ_FILE}"; then
  ...
  echo "[OK] Diffusion dependencies installed"
fi
